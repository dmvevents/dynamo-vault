#!/bin/bash
# Build script for LIBFABRIC backend Docker image
#
# Usage:
...
echo "=== Building TRT-LLM + Dynamo with LIBFABRIC backend ==="
...
if [[ ! -f "patch_nixl_connect.py" ]]; then
    echo "ERROR: patch_nixl_connect.py not found in $SCRIPT_DIR"
    exit 1
fi
